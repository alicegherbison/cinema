...
function ignoreSubtitled(showing) {
  if(!(showing.hasOwnProperty('properties') && showing.properties.hasOwnProperty('event.film.subtitled'))) {
    return true;
  }
}

function nextWeekOnly(showing) {
  const currentDate = new Date();
  const showingDate = new Date(showing.ts);
  const currentDay = currentDate.getDay();
  const showingDay = showingDate.getDay();
}
